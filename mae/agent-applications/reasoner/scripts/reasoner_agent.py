    ...
    def on_event(
        ...
        if dora_event["type"] == "INPUT":
            ...
            if dora_event["id"] in agent_inputs:
                ...
                results = {'agent_name': inputs.get('agent_name', 'reasoner'),'dataflow_status':True}
                write_agent_log(log_type=inputs.get('log_type',None),log_file_path=inputs.get('log_path',None),data=log_result)
                results['node_results'] = result
                print('agent_output:',results)
                # send_output("reasoner_result", pa.array([json.dumps(results)]),dora_event['metadata'])
                send_output("reasoner_result", pa.array([create_agent_output(step_name='reasoner_result', output_data=result,dataflow_status=os.getenv("IS_DATAFLOW_END", True))]), dora_event['metadata'])
