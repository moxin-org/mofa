...
class ResearchGenerator:
    ...
    def __init__(self, articles: List[ArticleRef], llm_client: Optional[LLMClient] = None, max_output: int = 20):
        ...
        self.phase_prompts = [
            ...
            "Several studies suggest different perspectives on this topic, such as...",
            "In conclusion, what are the primary takeaways from this research?",
            "What are the gaps in the current studies and how might they be addressed?",
            "How do these findings compare with previous studies or common assumptions?",
            "What new directions or innovations do these findings suggest for future research?"
        ]
    ...
    def _llm_think(self, stage_description: str, selected_articles: List[ArticleRef], stage_id: int, substep_id: int):
        ...
        for sub_stage_id,chunk in enumerate(self.llm_client.generate_response(messages)):
            yield {
                ...
                "metadata": {"stage": stage_description},
    ...
    def _llm_generate_content(self, prompt: str, related_articles: List[ArticleRef], stage_id: int, substep_id: int,stage:str=None,type_content:str=None):
    ...
    def generate_stream(self):
        ...
        for stage_id,stage in enumerate(self.thinking_stages):
            ...
            for chunk in self._llm_think(stage["description"], context_articles, stage_id, substep_id):
        ...
        for i, prompt in enumerate(self.phase_prompts):
            ...
            for article in related:
                self.used_articles.add(article.url)
            llm_content = ''
            for chunk in self._llm_generate_content(prompt + '\n This is the content of the previous Think Summary'+think_summary + f'\n This is question {user_query}', related, stage_id, substep_id):
        ...
        final_synthesis = self._llm_generate_content(final_prompt, self.articles, 0, 0,'summary','completion')
