...
#                 messages=[
#                     {"role": "system", "content": "你是一个智能 agent，能决定是否调用相关的工具来完成任务。"},
...
planner_llm = OpenAI(
    base_url="http://127.0.0.1:8080/v1",
    api_key="ollama",  # 对于 Ollama，api_key 可以是任意非空字符串，通常是 "ollama"
)
            ...
            if llm_message.tool_calls:
                ...
                for func_call in llm_message.tool_calls:
                    ...
                    try:
                        ...
                        t_call_tool = time.time()
                        tool_result_obj = await client.call_tool(tool_name, args)
