    ...
    def on_event(
        ...
        if dora_event["type"] == "INPUT":
            ...
            if self.reasoner_response is not None and self.task is not None:
                ...
                send_output("memory_log", pa.array([create_agent_output(agent_name='memory_record', agent_result={"reasoner_response":self.reasoner_response}, dataflow_status=os.getenv('IS_DATAFLOW_END', True))]), dora_event['metadata'])
                self.reasoner_response = None
                self.task = None
