...
class Operator:
    ...
    def on_event(
        ...
        if dora_event["type"] == "INPUT":
            ...
            if dora_event['id'] == "context_memory": self.context_memory = load_node_result(
                dora_event["value"][0].as_py())
            if self.task is not None and self.context_memory is not None:
                ...
                record_agent_result_log(agent_config=inputs,
                                        agent_result={"2, " + inputs.get('log_step_name', "Step_one"): agent_result})
