...
| **文件**                               | **作用**                                                                 |
| -------------------------------------- | ------------------------------------------------------------------------ |
| `code_assistant_copy_agent    .yml`         | 根据用户给出的问题即代码片段，给出新的代码。                        |
...
### 注意
如果你希望将你的代码作为上下文传递给智能体，你需要将你的代码放入code_assistant_copy目录(不支持子目录)，并以.py文件命名。
并且所有直接放入code_assistant_copy目录下的.py文件都会作为智能体的上下文
