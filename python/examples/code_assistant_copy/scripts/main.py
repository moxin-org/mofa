    ...
    for event in node:
        if event["type"] == "INPUT":
            if event['id'] in ['output']:
                ...
                node.send_output("code_assistant_copy_result", pa.array([create_agent_output(agent_name='code_assistant_copy',
                ...
                yaml_file_path = get_relative_path(current_file=__file__, sibling_directory_name='configs',
                                                target_file_name='code_assistant_copy_agent.yml')
                ...
                root.destroy()
                #print(inputs)
                node.send_output("get_code", pa.array([create_agent_output(agent_name='code_assistant_copy',
                                                                           agent_result=agent_result,
                                                                           dataflow_status=os.getenv(
                                                                                        "IS_DATAFLOW_END",
                                                                                        True))]), event['metadata'])
