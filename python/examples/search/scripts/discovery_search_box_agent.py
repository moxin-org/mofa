...
class Operator:
    ...
    def on_event(self, dora_event, send_output) -> DoraStatus:
        if dora_event["type"] == "INPUT" and dora_event['id'] == "load_url_connector_response":
            html_content = load_node_result(dora_event["value"][0].as_py())
            ...
            search_box_list = find_search_box(html_content, llm_client=llm_client)
            discovery_search_box_result = {'search_box_list': search_box_list, 'html_content': html_content}
