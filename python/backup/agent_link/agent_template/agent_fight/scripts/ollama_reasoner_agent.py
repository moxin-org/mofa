...
class Operator:
    def on_event(
        ...
        if dora_event["type"] == "INPUT":
            agent_inputs = ['reasoner_task','task_input','data','gpt_reasoner_result']
            if dora_event["id"] in agent_inputs:
                dora_result = json.loads(dora_event["value"][0].as_py())
                task = dora_result.get('task')
                yaml_file_path = get_relative_path(current_file=__file__, sibling_directory_name='configs', target_file_name='ollama_reasoner_agent.yml')
                ...
                results['task'] = task
                results['ollama_result'] = result
                results['gpt_result'] = dora_result.get('gpt_result')

                print('agent_output:',results)
                send_output("ollama_reasoner_result", pa.array([json.dumps(results)]),dora_event['metadata'])
