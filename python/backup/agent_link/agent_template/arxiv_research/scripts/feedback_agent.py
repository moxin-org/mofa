...
class Operator:
    def on_event(
        ...
        if dora_event["type"] == "INPUT":
            agent_inputs = ['writer_report','evaluation_result','evaluation_result']
            if dora_event['id'] in agent_inputs :
                task_inputs, dora_result, task = load_dora_inputs_and_task(dora_event)
                yaml_file_path = get_relative_path(current_file=__file__, sibling_directory_name='configs', target_file_name='feedback_agent.yml')
                inputs = load_agent_config(yaml_file_path)
                writer_result = dora_result
